...
- `gripper_type`: 可选，默认 `"angular_direct"`。RoArm-M3 仅一种夹爪，一般无需设置
...
- **参数:**
  ...
  - `torques:` [0, 1000], type : list[int]，unit : 0.1% 堵转扭矩（1000 = 100%）
    - `[关节1力矩阈值, 关节2力矩阈值, 关节3力矩阈值, 关节4力矩阈值, 关节5力矩阈值, 关节6力矩阈值]`
    - 超过该阈值时，关节随外力转动

####  `feedback_get()`
- **功能:** 获取末端坐标、姿态与各关节弧度反馈（需串口；HTTP 模式不支持读反馈）

- **返回值:** list[float]
  - `[x, y, z, pitch, 关节1弧度, 关节2弧度, 关节3弧度, 关节4弧度, 关节5弧度, 关节6弧度]`
  - 坐标 `x/y/z`：单位 毫米
  - `pitch`：末端姿态，单位 弧度
  - 关节弧度：单位 弧度
...
- **参数:**
  ...
  - `radian:` type : float，unit : 弧度
    ...
    - 3: 关节3，[0, 3.1415926]
    - 4: 关节4，[-1.5707963, 1.5707963]
    - 5: 关节5，[-3.1415926, 3.1415926]
    - 6: 关节6，[0, 1.5707963]
